...
if($_SERVER['HTTP_HOST'] != 'webmuni') {
	...
	if($lista_archivos) {
		while(!feof($lista_archivos)) {
			...
			if($archivo) {
				echo "<li>$archivo: ";
				$contenido = file_get_contents('http://192.168.100.5/inspeccion/' . $archivo . '.yuf');
				if($contenido) {
					echo " actualizado";
