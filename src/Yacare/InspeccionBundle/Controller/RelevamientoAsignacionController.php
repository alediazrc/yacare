...
class RelevamientoAsignacionController extends \Tapir\BaseBundle\Controller\AbmController
{
    ...
    {
        return $this->redirect($this->generateUrl($this->obtenerRutaBase('listar'), $this->ArrastrarVariables(array(
    ...
    public function guardarAction(Request $request, $id = null)
    {
        ...
        if ($editForm->isValid()) {
            ...
            if ($partidas) {
                ...
                foreach ($partidas as $partida) {
                    ...
                    if ($partida->getDomicilioCalle()) {
                        $Deta->setPartidaCalleNombre($partida->getDomicilioCalle()
                            ->getNombre());
                    }
            ...
            return $this->redirect($this->generateUrl(strtolower('yacare_' . $this->BundleName . '_' . $this->EntityName . '_listar'), array(
                'filtro_relevamiento' => $entity->getRelevamiento()
                    ->getId()
            )));
        }
