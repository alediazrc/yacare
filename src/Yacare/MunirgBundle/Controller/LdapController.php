    ...
    public function importar2Action(Request $request)
    {
       ...
       \ldap_set_option($ServidorAd, LDAP_OPT_REFERRALS, 0);
        ...
        if ($UsrBind) {
            ...
            $this->get('session')
                ->getFlashBag()
                ->add('success', '<i class="fa fa-check fa-2x"></i> Se asoció la cuenta con el agente legajo Nº ' . $AgenteLegajo . ' y se importaron los datos del servidor LDAP. Por favor intente conectarse nuevamente.');
