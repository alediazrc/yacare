...
class Cbu
{
    ...
    public static function EsCbuValida($Cbu)
    {
        $solonumeros = str_replace(array('.', ',', ' ', '-'), '', $Cbu);
        if (strlen($solonumeros) == 22) {
            $digitoVerificador = $solonumeros[7];
            $suma = $solonumeros[0] * 7 + $solonumeros[1] * 1 + $solonumeros[2] * 3 + $solonumeros[3] * 9 +
                 $solonumeros[4] * 7 + $solonumeros[5] * 1 + $solonumeros[6] * 3;
    ...
    public static function FormatearCbu($Cbu)
    {
        $solonumeros = str_replace(array('.', ',', ' ', '-'), '', $Cbu);
